...
function TelemetriaPage() {
  ...
  const filteredLogs = useMemo(() => {
    ...
    return logs.filter((log) => {
      ...
      const matchesSearch =
        log.job.toLowerCase().includes(search.toLowerCase()) ||
    ...
    const grouped = failures.reduce(
      (acc, curr) => {
        ...
        if (new Date(curr.started_at) > new Date(acc[route].last))
          acc[route].last = curr.started_at;
        return acc;
      },
      ...
      <div className="flex items-center justify-between mb-8 print:hidden">
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            Monitoramento de eventos, falhas e integridade da plataforma.
          </p>
        </div>
      ...
      {isError && (
        ...
        >
          ...
          <p className="mt-2 text-xs text-muted-foreground">
            {(error as Error)?.message ?? "Erro inesperado ao ler os registros."} Isto não significa
            que não há eventos: a leitura falhou.
          </p>
          <Button
            variant="outline"
            size="sm"
                ...
                <div key={f.route} className="flex justify-between items-center text-xs">
                  ...
                  <div className="flex gap-4">
                    ...
                    <span className="text-muted-foreground">
                      Última: {new Date(f.last).toLocaleTimeString()}
                    </span>
                  </div>
                </div>
              ))}
            ...
            {isLoading ? (
              ...
              <tr>
                <td colSpan={4} className="px-6 py-8 text-center text-muted-foreground">
                  Leitura indisponível — use “Tentar novamente”.
                </td>
              </tr>
